  ...
  function renderAirportCard(container, icao, fix, data, weatherData, icon) {
    ...
    if (procs.length > 0) {
      ...
      procs.forEach(function(p) {
        ...
        if (isDeparture) { html += '<tr><td>' + escapeHtml(p.name) + '</td><td>' + escapeHtml(rwy || '\u2014') + '</td><td>' + escapeHtml(trans) + '</td><td>' + escapeHtml(fix) + '</td></tr>'; } else { var apps = p.approaches || []; if (apps.length > 0) { apps.forEach(function(a,ai){ if (ai===0) html += '<tr><td>' + escapeHtml(p.name) + '</td>'; else html += '<tr><td></td>'; var aname = a.arinc_name ? a.name + ' (' + a.arinc_name + ')' : a.name; html += '<td>' + escapeHtml(aname) + '</td><td>' + escapeHtml(a.transition || '\u2014') + '</td><td>' + escapeHtml(a.runway || '\u2014') + '</td><td>' + escapeHtml(fix) + '</td></tr>'; }); } else { html += '<tr><td>' + escapeHtml(p.name) + '</td><td>\u2014</td><td>\u2014</td><td>' + escapeHtml(rwy || '\u2014') + '</td><td>' + escapeHtml(fix) + '</td></tr>'; } }
      });
